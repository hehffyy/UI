...
window.layer = {
    ...
    path: ready.getPath(),
    //载入模块
    use: function(module, isDir, callback ){
        var i = 0, head = $('head')[0];
...
Class.pt.space = function(html){
    ...
    if(config.maxmin && (config.type === 1 || config.type === 2) && (!/^\d+%$/.test(config.area[0]) || !/^\d+%$/.test(config.area[1]))){
        closebtn = '<a class="xubox_min" href="#"><cite></cite></a><a class="xubox_max xulayer_png32" href="#"></a>';
    }
    config.closeBtn[1] && (closebtn += '<a class="xubox_close xulayer_png32 xubox_close' + config.closeBtn[0] +'" href="#" style="'+ (config.type === 4 ? 'position:absolute; right:-3px; _right:7px; top:-4px;' : '') +'"></a>'); 
...
Class.pt.creat = function(){
    ...
    if(config.title && (config.type !== 3 || config.type !== 4)){
        ...
        switch(confbtn.btns){
            ...
            break;
            case 1:
                layerBtn.html('<a href="#" class="xubox_yes xubox_botton1">'+ confbtn.btn[0] +'</a>');
            break;
            case 2:
                layerBtn.html('<a href="#" class="xubox_yes xubox_botton2">'+ confbtn.btn[0] +'</a>' + '<a href="#" class="xubox_no xubox_botton3">'+ confbtn.btn[1] + '</a>');
            break;                
        }
    }
...
Class.pt.callback = function(){
    ...
    layerE.find('.xubox_close').bind('click', function(){
        config.close(that.index);
        layer.close(that.index);
    });
    
    layerE.find('.xubox_yes').bind('click',function(){
        config.yes ? config.yes(that.index) : dialog.yes(that.index);
    });
    
    layerE.find('.xubox_no').bind('click',function(){
        config.no ? config.no(that.index) : dialog.no(that.index);
        layer.close(that.index);
    });
    
    if(that.config.shadeClose){
        $('#xubox_shade'+ that.index).bind('click', function(){
            layer.close(that.index);
        });
    } 
    
    //最小化
    layerE.find('.xubox_min').bind('click', function(){
        layer.min(that.index, config);
        config.min && config.min(layerE);
    });
    
    //全屏/还原
    layerE.find('.xubox_max').bind('click', function(){
