	...
	public static Part[] generateParts(HttpServletRequest request) throws FileUploadException, IOException {
		...
		while (iterator.hasNext()) {
			...
			if (!fi.isFormField()) {
				...
				String fullFileName = FileSystemWrapper.instance().getRealPath(fieldValue);
				FileInputStream fis = new FileInputStream(fullFileName);
				InputStreamPartSource bps = new InputStreamPartSource(fis, fullFileName);
				partList.add(new FilePart(fieldName, bps));
			} else {
				String fieldValue = fi.getString();
				try {
				...
				if (0 != fieldName.indexOf("$"))
					partList.add(new StringPart(fieldName, fieldValue, "UTF-8"));
			}
		}
