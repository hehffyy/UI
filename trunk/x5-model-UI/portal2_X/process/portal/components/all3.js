...
(function(global){
	...
	FuncFrame3.prototype.resize = function(){
		...
		this.frame.height(this.el.height() - 5);/*fixed iframe bug*/
		if(this.frame.height()){
			$(".left").height(this.frame.height()).parent().height(this.frame.height());
		}
	};
...
(function(global){
	...
	AccordionMenuTree.prototype.load = function(options){
		var html = [], me = this;
		html.push('<div class="js-accordion-menu accordionTop_in" title="' + options.label + '">'+options.label+'</div>');
		//console.dir(options.childNodes);
		function draw(items, path){
			...
			$.each(items, function(i, item){
				...
				if(item.icon16)
					html.push('<li class="level1" title="' + item.label + '">');
				else
					html.push('<li title="' + item.label + '">');
				html.push('<label data-path="' + path.join(' ') + '" data-isdir="' + isDir + '" isIcon="' + (item.icon16 || '') + '">');
				if(isDir || item.icon16){
					if(item.icon16){
						var _icon = "acc-icon-"+item.icon16;
						//html.push('<i class="icon-system-right-dir acc-icon-default' + item.icon16 + '"></i>');
						html.push('<i class="acc-icon-default ' + _icon+ '"></i>');
						html.push('<span class="menuText">' + item.label + '</span>');
					}
				}
				if(!item.icon16){
					html.push('<i class="acc-icon-default"></i>');
		...
		this.el.append('<div class="slideControl slideIn"><span><i></i></span></div>');
		
		function funLableTriggerHandler(_this){
			var _self = _this;
			var isDir = $(_self).attr('data-isdir') == 'true';
			var isIcon = $(_self).attr('isIcon') != "";
			var p = $(_self).parent();
			//如果是一级菜单 显示下面的东西
			if(isIcon){
				$(".accordionMenuTree>ul ul").removeClass("js_selected");;
				$(_self).siblings("ul").addClass("js_selected");
				$(".accordionMenuTree>ul ul:not(.js_selected)").hide();
				$(_self).siblings("ul").slideToggle();
				
				if(!p.hasClass("level1"))
					return;
			}
			if(isDir){
				//计算三级菜单标题宽度
				...
				$('.ran', _self.el).removeClass('ran');
				p.addClass('ran');
				
	        	p.removeClass("le_in");
	        	p.removeClass("le_out");
	        	p.parent("ul").children("li").removeClass('le_in').removeClass('le_out');
	        	var _div = p.parent("ul").siblings(".slideControl");
	        	if(_div.hasClass('slideOut')){
				    p.addClass("le_in");
	        	}else{
	        		p.addClass("le_out");
	        	}
				...
				if(me.onclick){
					var path = $(_self).attr('data-path').split(' ');
					var data = options;
		...
		function toggleSlideUp(_that){
			// 进行收缩
			_that.siblings(".js-accordion-menu")
			       .removeClass("accordionTop_in")
			       .addClass("accordionTop_out")
			       .text("");
			
			_that.siblings("ul").children("li").children("label").removeClass("accMenu_lable_out").addClass("accMenu_lable_in");;
			
			var _dv = _that.siblings("ul").children("li");
			_dv.removeClass("le_out").removeClass("le_in");
			if(_dv.hasClass('ran')){
				_dv.filter(".ran").addClass("le_in");
			}
			
			_that.removeClass("slideIn").addClass("slideOut");
			$(".left").removeClass("menu-normal").addClass("menu-min");
			$(".menuText").hide();
			$(".accordionMenuTree>ul ul").hide();
			//绑定移动方法
			$(".menu-min>ul>li").mouseover(function(){
        ...
        function toggleSlideDown(_that){
        	// 进行伸展
        	_that.siblings(".js-accordion-menu")
			       .removeClass("accordionTop_out")
			       .addClass("accordionTop_in").text(_that.siblings(".js-accordion-menu").attr("title"));
        	_that.removeClass("slideOut").addClass("slideIn");
			
        	_that.siblings("ul").children("li").children("label").removeClass("accMenu_lable_in").addClass("accMenu_lable_out");
        	
        	var _dv = _that.siblings("ul").children("li");
        	_dv.removeClass("le_in");
        	_dv.removeClass("le_out");
        	if(_dv.hasClass('ran')){
			    _dv.filter(".ran").addClass("le_out");
        	}
        	
			$(".menuText").show();
			$(".accordionMenuTree>ul ul").hide();
			$(".left").removeClass("menu-min").addClass("menu-normal");
			
			//解绑移动方法
			$(".menu-normal>ul>li").unbind("mouseover").unbind("mouseleave");
		...
		$(".slideControl").click(function(){
			if($(this).hasClass("slideIn")){
				...
				// 进行收缩
				toggleSlideDown($(this));
				$(this).siblings("ul").children("li").children("ul .js_selected").show();
			}
		});
		...
		if($(".slideControl").hasClass("slideIn")){
			// 进行收缩
			toggleSlideDown($(".slideControl"));
			// 默认打开第一个目录下的第一个功能
			var $li = $(".accordionMenuTree>ul").children("li").first().children("label");
			if(!($li.attr("data-isdir")==="true")){
				// 打开第一个
				$li.trigger("click");
			}else{
				// 打开li下面的lable click
				$li.trigger("click");
				$li.siblings("ul").children("li").first().children("label").trigger("click");
			}
		}else{
			// 进行伸展
			toggleSlideUp($(".slideControl"));
		}
	};
	...
	AccordionMenuTree.funToggleSlideDown = function(){
		toggleSlideUp($(".slideControl"));
	};
