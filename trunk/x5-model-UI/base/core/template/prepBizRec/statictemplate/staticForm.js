		...
		function(require, jquery) {
			...
			StaticForm.prototype.init = function(param) {
				...
				if (parent) {
					...
					var bizRecId = param.bizRecId || param.rowid 
					|| justep.Context.getRequestParameter("bizRecId") || justep.Context.getRequestParameter("rowid") 
					...
					this.attachEvent(bizRec, justep.XData.EVENT_SAVEDATA_AFTER,
				...
				this._openDatas(parent ? parent.BizRec : this.BizRec,
						cacheDatas);
			 ...
			 *            父表单的缓存数据
			 */
			StaticForm.prototype._openDatas = function(parentBizRec, cacheDatas) {
			...
			StaticForm.prototype._handleDataChangedEvent = function(event) {
				...
				if (codeFieldManager) {
					...
					if (event.type == "delete") {
						...
						if (frameForm.__inited && !event.cloneData
								&& event.success) {
							codeFieldManager
			...
			StaticForm.prototype.saveDatas = function(options) {
				var mainForm = this.getRootForm();
				// 保存主窗体的案卷表
				return mainForm.promiseRefreshControls().done(
						function() {
			 ...
			 * @param event
			 */
			StaticForm.prototype._doSaveCacheDatas = function() {
				this.__saveDatas = [];
			 ...
			 * @param event
			 */
			StaticForm.prototype._setTabeLogicPluginSaveParam = function(event) {
				var BizRec = justep.xbl("BizRec");
				var variants = event.param.getParam("variants")
						|| new justep.Request.MapParam();
				variants.put("tableName", event.source.id);
				variants.put("bizRecId", BizRec.getValue("fBizRecId"));
			...
			StaticForm.prototype.refreshBizRecSaveState = function() {
				...
				if (state && state != justep.XData.STATE.NONE
					...
					// 案卷虚拟编辑中或者未改变
					var haveChangedData = this.currentForm.BizRec
							.isChanged("all");
					if (haveChangedData) {
						bizRec.setOperationEnable("save", true);
						if (bizRec.__virtualEdit) {
