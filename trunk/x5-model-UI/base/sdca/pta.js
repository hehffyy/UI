...
(function(global) {
// global const
var iePtaObject = "<object id='iTrusPTA' codebase='download/itruscertctl.cab#version=2,5,5,6' classid='clsid:1E0DFFCF-27FF-4574-849B-55007349FEDA'></object>";
var ffPtaObject = "<object id='iTrusPTA' type='application/PTA.iTrusPTA'></object>";
 ...
 *            导出证书文件的备份密码
 * @param opt
 *            导出参数 opt = opt | EXPORT_DISABLE; //是否允许再导出？ opt = opt |
 *            EXPORT_DELETE; //导出成功后是否从系统删除原证书？
...
function signLogonData(signer, inputToSign) {
	try {
		...
		if (ptaVersion == null) {
			// PTA Version = 1.0.0.3
			signedData = signer.SignMessage(inputToSign, OUTPUT_BASE64);
		} else {
			// PTA Version > 2
			if (inputToSign.indexOf("LOGONDATA:") == -1)
				inputToSign = "LOGONDATA:" + inputToSign;
			signedData = signer.SignLogonData(inputToSign, OUTPUT_BASE64);
		}
		return signedData;
	} catch (e) {
 ...
 */
var SDCA = {
	 arrayIssuerDN : "O=山东省国土资源厅, OU=信息中心, CN=山东省国土资源厅 User CA"
	};
global.SDCA = SDCA;
 ...
 */
SDCA.getCertInfo=function(){
	var arrayCerts = filterCerts(this.arrayIssuerDN, 1, "");
...
SDCA.getCaKeyId=function(){
	var cert = this.getCertInfo();
	if(cert)
		return cert.SerialNumber;
};

SDCA.getUserName=function(){
	var cert = this.getCertInfo();
	if(cert)
