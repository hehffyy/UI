...
public class EditMappingPage extends Composite implements IPropertyDialogPage {
	...
	public void setPropertyItem(PropertyItem arg0) {
		...
		try {
			String url = xblElement.getProperyValue("targetUrl");
	...
	public Document parseExtendWindow(String base, String windowURL) {
		try {
			File libFile = new File(StudioConfig.getWindowExtendEngineBasePath() + "/lib");
	...
	public void createMappingXMLByTableData() {
		...
		if (dataset.getDataRecords().size() > 0) {
			...
			dataXML.append("<result concept =\"").append(conceptCombo2.getText()).append("\" operation = \"").append(operation)
					.append("\" origin = \"").append(conceptCombo1.getText()).append("\">");
			...
			for (DataRecord dr : drList) {
				...
				dataXML.append("<mapping from=\"").append((dr.getString("from"))).append("\" to=\"").append((dr.getString("to"))).append("\"")
