define(
		...
		function(FrameForm) {
			...
			FuncManager.prototype.initFormNav = function() {
				var defaultForm, navs = this.config.navs, $container = $('#formNavs'), cnt = 0;
				...
				for ( var navIdx in navs) {
					...
					for ( var formIdx in nav.forms) {
						...
						if (visibleForms != null
							...
							continue;
						if (!defaultForm
								&& (config.name == decodeURIComponent(justep.Request.URLParams["form"]) || !justep.Request.URLParams["form"]
										&& config.isDefault)) {
							defaultForm = config;
						}
						// title属性用于setFormVisible
						var $li = $("<a class='list-icon-link' title='"
								+ nav.name + "'>" + (++cnt) + "</a>");
						$container.append($li);
						config.jQNode = $li;
						...
						$li.bind("click", $.proxy(function(event) {
							if ($(event.currentTarget).hasClass(
									"list-icon-current"))
								return;
							var prev = $(".list-icon-current", $container);
							if (prev.length > 0) {
								// 如果前一个表单正在加载中
								if (!prev.data("config").form)
									return;
								prev.removeClass("list-icon-current");
							}
			...
			FuncManager.prototype.showForm = function(config) {
				...
				if (prevForm) {
					...
					if (this.mainForm.formDoc.checkEvent("onBeforeChangeForm")) {
						...
						this.mainForm.formDoc.callEvent("onBeforeChangeForm",
								[ event ]);
						if (event.cancel) {
							prevForm.config.jQNode
									.addClass('list-icon-current');
							return;
						}
				...
				config.jQNode.addClass('list-icon-current');
				if (config.form) {
