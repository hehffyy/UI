define(
		"base/lib/bind/observable",
		[ require, "./util", "./array", "./object" ],
		function(require, Util, _Array, Object) {
			var Event = Object
					.extend({
						...
						addListener : function(fn, scope, options) {
							...
							if (!me.isListening(fn, scope)) {
								listener = me
										.createListener(fn, scope, options);
								if (me.firing) {
									// if we are currently firing this event,
									// don't
									// disturb the
									// listener loop
						...
						removeListener : function(fn, scope) {
							...
							if (index != -1) {
								...
								// cancel and remove a buffered handler that
								// hasn't
								// fired yet
								if (listener.task) {
								...
								// cancel and remove all delayed handlers that
								// haven't
								// fired yet
						...
						clearListeners : function() {
							...
							while (i--) {
								this.removeListener(listeners[i].fn,
										listeners[i].scope);
							}
						},
						...
						fire : function() {
							...
							if (count > 0) {
								...
								for (i = 0; i < count; i++) {
									listener = listeners[i];
									args = arguments.length ? Array.prototype.slice
											.call(arguments, 0)
											: [];
									if (listener.o) {
			...
			var Observable = Object
					.extend({
						...
						on : function(ename, fn, scope, options) {
							...
							if (typeof ename !== 'string') {
								...
								if (Util.isBoolean(event)) {
									me.events[ename] = event = new Event(this,
											ename);
								}
								event.addListener(fn, scope, Util
						 ...
						 */
						fireEvent : function() {
							var me = this, args = Util.toArray(arguments), ename = args[0]
									.toLowerCase(), ret = true, event = me.events[ename];
