define(
		"base/lib/bind/viewComponent",
		[ "require", "jquery", "bind", "./util", "./object", "./message",
				"./error", "./component" ],
		function(require, $, Bind, Util, Object, Message, _Error, Component) {
			...
			var BIND_EVENTS = [ "bind-click", "bind-dbclick", "bind-dblclick",
					"bind-focus", "bind-blur", "bind-select", "bind-mousedown",
					"bind-mouseup", "bind-mouseover", "bind-mousemove",
					"bind-mouseout", "bind-mouseenter", "bind-mouseleave",
					"bind-keydown", "bind-keyup", "bind-keypress",
					"bind-touchstart", "bind-touchmove", "bind-touchend",
					"bind-abort", "bind-error", "bind-change", "bind-reset",
					"bind-resize", "bind-submit", "bind-load", "bind-unload" ];
			var ViewComponent = Component
					.extend({
						...
						constructed : function(config) {
							...
							if (config && config.templateNode) {
								...
								if (!!template) {
									...
									// TODO
									// 只处理了events和binds，按合理的角度，还要处理properties;
									// 出于性能优化的考虑,
									// 要求组件开发者在buildTemplate中处理properties
						...
						_processComponentBind : function(config) {
							...
							var e = $(this.domNode);
							var component = e
									.attr(Component.COMPONENT_ATTR_NAME);
							if (component) {
						...
						_processEventValue : function(value) {
							if (value && (value.indexOf(".") == -1)
									&& (value.indexOf("(") == -1)) {
								return "$model._callModelFn.bind($model, '"
										+ value + "')";
							} else {
								return value;
							}
						...
						_processBind : function(template, config) {
							...
							var eventBinds = "";
							var items = this
									._getStandardBinds(template, config);
							for ( var name in items) {
								if (items.hasOwnProperty(name)) {
									var value = items[name];
									if (BIND_EVENTS.indexOf(name) != -1) {
										localname = name
												.substring(BIND_PREFIX.length);
										if (eventBinds)
											eventBinds += ",";
										eventBinds += localname
												+ ":"
												+ this
														._processEventValue(value);

									} else if (name.indexOf(BIND_ATTR_PREFIX) === 0) {
										localname = name
												.substring(BIND_ATTR_PREFIX.length);
										if (attrBinds)
											attrBinds += ",";
						...
						fireEvent : function(name, evt) {// 重新实现，增加bindingContext
							if (evt && this.domNode
									&& !evt.hasOwnProperty('bindingContext'))
								evt['bindingContext'] = Bind
						...
						dispose : function() {
							if (this.domNode) {
								...
								Bind.utils.domData.set(this.domNode,
										Component.BIND_NAME, null);
								this.domNode = null;
								this.$domNode = null;
							}
							this.callParent();
						...
						init : function(value, bindingContext) {
								...
								for ( var i = 0; i < items.length; i++) {
									...
									if (item !== "") {
										...
										if (index != -1) {
											...
											var msg = new Message(
													'{"code":"JUSTEP230065", "message":"组件事件{0}格式非法"}',
													events);
							...
							for ( var prop in this.__events) {
								if (this.__events.hasOwnProperty(prop)) {
									this.on(prop, this.__events[prop],
											viewModel);
								}
							}
						...
						update : function(value, bindingContext) {
							...
							for (name in value) {
								if (value.hasOwnProperty(name)) {
									values[name] = Bind.utils
											.unwrapObservable(value[name]);
								}
							}
						...
						getElementByCompID : function(compId) {
							return this._getElementByCompID(this.domNode,
									compId);
						},
						...
						_getElementByCompID : function(element, compId) {
							...
							if (!elementObj.attr(Component.COMPONENT_ATTR_NAME)) {
								...
								for ( var i = 0; i < children.length; i++) {
									...
									var result = this._getElementByCompID(
											child, compId);
									if (result) {
										return result;
									}
						...
						hasDisplayFunc : function() {
							return 'function' === typeof (this.getDisplayHtml)
									|| 'function' === typeof (this.getDisplayText);
						},

						_bind : function(element) {
							Bind.utils.domData.set(element,
									Component.BIND_NAME, this);
							this.domNode = element;
							this.$domNode = $(this.domNode);
							Bind.utils.domNodeDisposal.addDisposeCallback(
									element, this.dispose.bind(this));
						}
					});
			...
			var DisplayContext = Object.extend({
				...
				getParam : function(name) {
					if (this.editor)
						return this.editor.get(name);
					else if (this.extendType)
						return this.extendType.getParam(name);
				}
			});
			...
			butone.Component.register("$model/UI/base/lib/bind/viewComponent",
					ViewComponent);
