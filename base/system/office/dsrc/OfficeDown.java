...
public class OfficeDown extends com.justep.ui.impl.JProcessorImpl  {
	...
	private void downWord(HttpServletRequest request,
		...
		try {
			...
			String parentVersion= request.getParameter("parentVersion"); 
			...
			ActionResult actionResult = executeAction("downOffice", params,bsessionid);
			if(!actionResult.isSuccess())
				throw new Exception(actionResult.getReason());
			if(actionResult.getDatas().get(0)==null)
				return;
			...
			response.reset();
			String df = version==null ? file.getName() : 
				(new String(version.getBytes("GB2312"), "ISO_8859_1") + (file.getName().indexOf(".")>0?file.getName().substring(file.getName().indexOf(".")):""));
			// 将 文件流写入到前端浏览器
			response.setHeader("Content-disposition",
					"attachment;filename=" + df);
